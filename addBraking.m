...
function start_braking_timestep = findBrakingTimestep(params, global_speeds, previous_arrival_timestep, braking_goal_speed)
    ...
    global_trajectory(2:numel(global_speeds)) = cumsum(global_speeds(1:end-1));

    approach_direction = sign(global_speeds(previous_arrival_timestep));
    % Only consider time since last direction change
    first_approach_idx = find(sign(global_speeds(1:previous_arrival_timestep-1)) == -approach_direction, 1, 'last') + 1;
    ...
    candidate_timesteps = first_approach_idx:previous_arrival_timestep;
