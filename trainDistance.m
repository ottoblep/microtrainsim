function distance = trainDistance(network, traj_set, i_train, j_train, timestep)
    ...
    edge_i = int32(traj_set(i_train, timestep, 1));
    edge_j = int32(traj_set(j_train, timestep, 1));

    i_edge_length = network.edge_values(edge_i);
    j_edge_length = network.edge_values(edge_j);

    if edge_i == edge_j
        distance = abs(traj_set(i_train, timestep, 2) - traj_set(j_train, timestep, 2)) * i_edge_length;
    else
        ...
        i_remaining_backward_length = traj_set(i_train, timestep, 2) * i_edge_length;
        i_remaining_forward_length = i_edge_length - i_remaining_backward_length;
        j_remaining_backward_length = traj_set(j_train, timestep, 2) * j_edge_length;
